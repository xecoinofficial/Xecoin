...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Xecoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
        {"138.68.131.104", "138.68.131.104"},
        {"138.68.131.210", "138.68.131.210"},
        {"139.59.16.18", "139.59.16.18"},
        {"139.59.16.104", "139.59.16.104"},
        {"46.101.148.45", "46.101.148.45"},
        {"46.101.100.9", "46.101.100.9"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("Xecoin-dnsseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Xecoin is probably already running."), addrBind.ToString().c_str());
        else
